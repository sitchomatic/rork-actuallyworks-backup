...
class AutomationActor {
    ...
    func runVisionCalibrateAndLogin(
        ...
        if let postScreenshot = await session.captureScreenshot() {
            ...
            let visionResult = await UnifiedAIVisionService.shared.analyzeScreenshot(postScreenshot, context: context)
            if visionResult.outcome == .success {
                logger.log("AutomationActor: AI Vision SUCCESS — \(visionResult.reasoning)", category: .automation, level: .success, sessionId: sessionId)
                completedTasks += 1
                return .success
            }
            logger.log("AutomationActor: AI Vision — \(visionResult.outcome) (\(visionResult.confidence)%) — \(visionResult.reasoning)", category: .automation, level: .info, sessionId: sessionId)
