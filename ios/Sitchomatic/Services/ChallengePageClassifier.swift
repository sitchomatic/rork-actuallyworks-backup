...
class ChallengePageClassifier {
    ...
    func classify(
        ...
        var ocrTextForBypass = ""
        var capturedScreenshot = screenshot
        if capturedScreenshot == nil {
            capturedScreenshot = await session.captureScreenshot()
        }
        if let img = capturedScreenshot {
            let context = VisionContext(site: "unknown", phase: .loginOutcome, currentURL: currentURL, attemptNumber: 1)
            let visionResult = await UnifiedAIVisionService.shared.analyzeScreenshot(img, context: context)

            if visionResult.isPageBlank {
                scores[.jsFailed, default: 0] += 0.8
                signals.append("AI_VISION: blank page detected")
            }

            if visionResult.outcome == .connectionFailure && visionResult.confidence >= 60 {
                scores[.temporaryBlock, default: 0] += 0.7
                signals.append("AI_VISION: connection failure (\(visionResult.confidence)%) — \(visionResult.reasoning)")
            }

            let ocrText = await UnifiedAIVisionService.shared.extractOCRText(from: img)
            ocrTextForBypass = ocrText
            let ocrLower = ocrText.lowercased()
            classifyFromDOM(contentLower: ocrLower, url: currentURL, signals: &signals, scores: &scores)
            classifyJSHealth(contentLower: ocrLower, pageLength: ocrText.count, signals: &signals, scores: &scores)
        } else {
            let fallbackContent = await session.getPageContent() ?? ""
            ocrTextForBypass = fallbackContent
            let contentLower = fallbackContent.lowercased()
            classifyFromDOM(contentLower: contentLower, url: currentURL, signals: &signals, scores: &scores)
            classifyJSHealth(contentLower: contentLower, pageLength: fallbackContent.count, signals: &signals, scores: &scores)
        }
