...
class UnifiedScreenshotManager {
    ...
    func addScreenshot(
        ...
        if runVisionAnalysis {
            ...
            let startTime = Date()
            let visionResult = await UnifiedAIVisionService.shared.analyzeScreenshot(compressedImage, context: VisionContext(site: site, phase: .loginOutcome))
            let ocrText = await UnifiedAIVisionService.shared.extractOCRText(from: compressedImage)
            let elapsedMs = Int(Date().timeIntervalSince(startTime) * 1000)

            let mappedOutcome = mapVisionOutcomeToDetectedOutcome(visionResult.outcome)
            analysis = VisionTextCropService.AnalysisResult(
                allText: ocrText,
                crucialMatches: visionResult.confidence >= 70 && visionResult.outcome != .noAcc ? [visionResult.reasoning] : [],
                detectedOutcome: mappedOutcome,
                confidence: Double(visionResult.confidence) / 100.0,
                textBlocks: [],
                processingTimeMs: elapsedMs
            )

            if let a = analysis, !a.crucialMatches.isEmpty {
                analysisStats.crucialDetections += 1
                let crop = await visionCrop.smartCrop(compressedImage)
            ...
            let outcomeKey = mappedOutcome.rawValue
    ...
    private func mapVisionOutcomeToDetectedOutcome(_ outcome: LoginOutcome) -> VisionTextCropService.DetectedOutcome {
        switch outcome {
        case .success: return .success
        case .permDisabled: return .permDisabled
        case .tempDisabled: return .tempDisabled
        case .noAcc: return .noAccount
        case .smsDetected: return .smsVerification
        case .connectionFailure: return .errorBanner
