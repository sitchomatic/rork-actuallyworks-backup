...
class ProxyRotationService {
    ...
    private func loadVPNConfigs() {
        ...
        ignitionVPNConfigs = []
        ppsrVPNConfigs = []
        clearAllPersistedVPNConfigs()
    }

    private func clearAllPersistedVPNConfigs() {
        for profile in NordKeyProfile.allCases {
            for target in [ProxyTarget.joe, .ignition, .ppsr] {
                let key = vpnPersistKey(for: target, profile: profile)
    ...
    private func loadWGConfigs() {
        ...
        ignitionWGConfigs = []
        ppsrWGConfigs = []
        clearAllPersistedWGConfigs()
    }

    private func clearAllPersistedWGConfigs() {
        for profile in NordKeyProfile.allCases {
            for target in [ProxyTarget.joe, .ignition, .ppsr] {
                let key = wgPersistKey(for: target, profile: profile)
                UserDefaults.standard.removeObject(forKey: key)
