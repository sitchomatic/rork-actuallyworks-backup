    ...
    private func checkSingleEmail(_ email: String) async -> DisabledCheckResult {
        for attempt in 1...3 {
            ...
            guard let screenshot = await session.captureScreenshot() else {
                session.tearDown(wipeAll: true)
                if attempt < 3 {
                    addLog("No screenshot for \(email), retrying (attempt \(attempt))", level: .warning)
                    try? await Task.sleep(for: .seconds(Double.random(in: 1.0...2.0)))
                    continue
                }
                return DisabledCheckResult(email: email, isDisabled: false, responseText: "Screenshot capture failed")
            }

            let currentURL = await session.getCurrentURL()
            let context = VisionContext(site: "joe", phase: .disabledCheck, currentURL: currentURL, attemptNumber: attempt)
            ...
            if result.outcome == .permDisabled {
                return DisabledCheckResult(email: email, isDisabled: true, responseText: "AI Vision: \(result.reasoning)")
            }

            if result.isPageBlank && attempt < 3 {
                addLog("Blank page for \(email), retrying (attempt \(attempt))", level: .warning)
                try? await Task.sleep(for: .seconds(Double.random(in: 1.0...2.0)))
                continue
            }

            return DisabledCheckResult(email: email, isDisabled: false, responseText: "AI Vision: \(result.reasoning)")
        }
