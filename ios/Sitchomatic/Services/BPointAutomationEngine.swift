...
class BPointAutomationEngine {
    ...
    private func performCardEntryAndSubmit(session: BPointWebSession, check: PPSRCheck, sessionId: String, deadline: Date) async -> CheckOutcome {
        ...
        var evaluation = await evaluateBPointViaAIVision(session: session, currentURL: currentURL, sessionId: sessionId, check: check)
        ...
        if evaluation.outcome == .uncertain {
            ...
            for pollIdx in 1...5 {
                ...
                let pollEval = await evaluateBPointViaAIVision(session: session, currentURL: pollURL, sessionId: sessionId, check: check)
        ...
        check.responseSnippet = "AI Vision: \(evaluation.reason)"
        ...
        default:
            ...
            check.completedAt = Date()
            onUnusualFailure?("BPoint uncertain for \(check.card.displayNumber): \(evaluation.reason)")
            return .uncertain
        }
    }
    ...
    private func evaluateBPointViaAIVision(session: BPointWebSession, currentURL: String, sessionId: String, check: PPSRCheck) async -> BPointEvaluation {
        guard let screenshot = await session.captureScreenshotWithCrop(cropRect: nil).full else {
            return BPointEvaluation(outcome: .uncertain, score: 0, reason: "No screenshot available for AI Vision")
        }

        let context = VisionContext(site: "bpoint", phase: .ppsr, currentURL: currentURL, attemptNumber: 1)
        let result = await UnifiedAIVisionService.shared.analyzePPSR(screenshot, context: context)

        logger.log("BPoint AI Vision: passed=\(result.passed) declined=\(result.declined) conf=\(result.confidence)% — \(result.summary)", category: .evaluation, level: result.passed ? .success : result.declined ? .warning : .info, sessionId: sessionId)
        check.logs.append(PPSRLogEntry(message: "AI Vision BPoint: \(result.summary) (\(result.confidence)%)", level: result.passed ? .success : result.declined ? .warning : .info))

        if result.passed && result.confidence >= 50 {
            return BPointEvaluation(outcome: .pass, score: result.confidence, reason: "AI Vision: \(result.summary)")
        }
        if result.declined && result.confidence >= 50 {
            return BPointEvaluation(outcome: .failInstitution, score: result.confidence, reason: "AI Vision: \(result.summary) [\(result.errorType)]")
        }

        return BPointEvaluation(outcome: .uncertain, score: result.confidence, reason: "AI Vision: \(result.summary)")
    }
