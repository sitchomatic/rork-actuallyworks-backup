...
class TrueDetectionService {
    ...
    nonisolated struct TrueDetectionConfig: Sendable {
        ...
        static func forSite(_ site: LoginTargetSite) -> TrueDetectionConfig {
            var config = TrueDetectionConfig()
            config.emailSelector = site.emailSelector
            config.passwordSelector = site.passwordSelector
            config.submitSelector = site.submitSelector
            return config
        }
    ...
    func validateSuccess(
        ...
        guard let screenshot = await session.captureScreenshot() else {
            onLog?("TRUE DETECTION: No screenshot for success validation", .warning)
            return false
        }

        let currentURL = await session.getCurrentURL()
        let context = VisionContext(site: "unknown", phase: .loginOutcome, currentURL: currentURL, attemptNumber: 1)
        let result = await UnifiedAIVisionService.shared.analyzeScreenshot(screenshot, context: context)

        if result.outcome == .success {
            onLog?("TRUE DETECTION: AI Vision SUCCESS — \(result.reasoning) (\(result.confidence)%)", .success)
            logger.log("TrueDetection: AI Vision success validated (\(result.confidence)%)", category: .evaluation, level: .success, sessionId: sessionId)
            return true
        }

        onLog?("TRUE DETECTION: AI Vision — \(result.outcome) (\(result.confidence)%) — \(result.reasoning)", .warning)
        return false
    }
    ...
    private func checkTerminalErrors(
        ...
        case .tempDisabled:
            onLog?("TRUE DETECTION: AI Vision TEMP DISABLED — \(result.reasoning)", .error)
            logger.log("TrueDetection: AI Vision tempDisabled (\(result.confidence)%)", category: .evaluation, level: .critical, sessionId: sessionId)
            return .temporarilyDisabled
        case .permDisabled:
            onLog?("TRUE DETECTION: AI Vision PERM DISABLED — \(result.reasoning)", .error)
            logger.log("TrueDetection: AI Vision permDisabled (\(result.confidence)%)", category: .evaluation, level: .critical, sessionId: sessionId)
            return .accountDisabled
        case .smsDetected:
            onLog?("TRUE DETECTION: AI Vision SMS DETECTED — \(result.reasoning)", .error)
            logger.log("TrueDetection: AI Vision smsDetected (\(result.confidence)%)", category: .evaluation, level: .critical, sessionId: sessionId)
            return .smsVerification
        case .noAcc where result.confidence >= 70:
            onLog?("TRUE DETECTION: AI Vision error detected — \(result.reasoning)", .warning)
            logger.log("TrueDetection: AI Vision noAcc with high confidence (\(result.confidence)%)", category: .evaluation, level: .warning, sessionId: sessionId)
            return .errorBanner
        default:
            return nil
        }
