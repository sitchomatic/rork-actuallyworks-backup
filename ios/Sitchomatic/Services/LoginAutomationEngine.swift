...
class LoginAutomationEngine {
    ...
    private func phasePatternCycleLoop(
        ...
        for cycle in 1...maxSubmitCycles {
            ...
            if cycle > 1 {
                ...
                if buttonReadyResult.ready {
                    ...
                    if !fallbackReady {
                        ...
                        attempt.status = .failed
                        attempt.errorMessage = "Login button hung in loading state — connection failure"
                        attempt.completedAt = Date()
            ...
            let pollResult = await session.rapidSuccessPoll(timeout: responseTimeout, originalURL: preSubmitURL)
            ...
            attempt.logs.append(PPSRLogEntry(
                message: "Success rapid poll: \(successTextFound ? "FOUND — \(successContext ?? "")" : "NOT FOUND")",
            ...
            if pollResult.errorBannerDetected {
                attempt.logs.append(PPSRLogEntry(
                    message: "Rapid poll hint: error banner detected — AI Vision will confirm",
                    level: .info
                ))
            }
            ...
            if pollResult.smsNotificationDetected {
                attempt.logs.append(PPSRLogEntry(
                    message: "Rapid poll hint: SMS notification detected — AI Vision will confirm",
                    level: .info
                ))
            }

            logger.startTimer(key: "\(sessionId)_eval_\(cycle)")
            let evaluation = await evaluateLoginResponseViaAIVision(
                session: session,
                screenshot: screenshotImage,
                pageContent: pageContent,
                currentURL: currentURL,
                preLoginURL: preLoginURL,
                successTextFound: successTextFound,
            ...
            case .unsure: autoResult = .unknown
    ...
    private func evaluateLoginResponseViaAIVision(
        session: LoginSiteWebSession,
        screenshot: UIImage?,
        pageContent: String,
        currentURL: String,
        ...
        guard let img = screenshot else {
            return EvaluationResult(
                outcome: .noAcc,
                score: 30,
                reason: "No screenshot — classified as No Account",
                signals: ["AI_VISION: no screenshot"]
            )
        }

        let context = VisionContext(site: "unknown", phase: .loginOutcome, currentURL: currentURL, attemptNumber: 1)
        let result = await UnifiedAIVisionService.shared.analyzeScreenshot(img, context: context)

        logger.log("AI Vision eval: \(result.outcome) (\(result.confidence)%) — \(result.reasoning)", category: .evaluation, level: result.outcome == .success ? .success : .info, sessionId: sessionId)

        return EvaluationResult(
            outcome: result.outcome,
            score: result.confidence,
            reason: "AI Vision: \(result.reasoning)",
            signals: ["AI_VISION: \(result.outcome) (\(result.confidence)%)"]
        )
    }
    ...
    private func captureAlwaysScreenshot(session: LoginSiteWebSession, attempt: LoginAttempt, cycle: Int, maxCycles: Int, successTextFound: Bool, redirected: Bool, evaluationReason: String, currentURL: String, autoResult: PPSRDebugScreenshot.AutoDetectedResult) async {
        ...
        let finalAutoResult = autoResult
    ...
    private func visionVerifyPostLogin(session: LoginSiteWebSession, sessionId: String) async -> (successFound: Bool, errorFound: Bool, context: String?) {
        ...
        let successFound = result.outcome == .success
        let errorFound = result.outcome == .noAcc || result.outcome == .permDisabled || result.outcome == .tempDisabled
        return (successFound, errorFound, result.reasoning)
    }
