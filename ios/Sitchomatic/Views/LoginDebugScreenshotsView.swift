    ...
    private var singleSiteStats: some View {
        HStack(spacing: 6) {
            ...
            if album.unsureCount > 0 {
                miniStat(icon: "xmark.seal.fill", count: album.unsureCount, color: .red)
            }
            Spacer()
...
struct LoginScreenshotCorrectionSheet: View {
    ...
    private var detectionReasoningSummary: String {
        ...
        if note.contains("lobby") || note.contains("dashboard") || note.contains("my account") { reasons.append("Detected success indicators → Success") }
