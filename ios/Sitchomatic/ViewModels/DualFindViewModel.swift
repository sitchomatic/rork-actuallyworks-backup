...
class DualFindViewModel {
    ...
    private let aiSettlement = AIVisionSettlementService.shared
    ...
    private func sessionEmailLoop(sessionIndex: Int, site: LoginTargetSite, emails: [String], password: String, passwordIndex: Int) async {
        ...
        while true {
            ...
            let siteName = site == .joefortune ? "joe" : "ignition"
            let settlementContext = VisionContext(site: siteName, phase: .settlement, currentURL: currentURL, attemptNumber: 1)
            ...
            log("V5.2 [\(label)] AI Settlement: \(settlement.reasoning) (\(settlement.durationMs)ms)", level: settlement.settled ? .info : .warning)
            ...
            if outcome == .unsure {
                outcome = .noAccount
                log("[\(label)] \(email) — legacy unsure result reclassified as No Account", level: .info)
            }
    ...
    private func evaluateV52Cascade(session: LoginSiteWebSession) async -> DualFindTestOutcome {
        ...
        case .smsDetected: return .fingerprintDetected
        case .noAcc: return .noAccount
        case .unsure: return .noAccount
    ...
    private func retryEmailOnFreshSession(session: LoginSiteWebSession, email: String, password: String, site: LoginTargetSite, sessionIndex: Int, label: String) async -> DualFindTestOutcome {
        ...
        let retryURL = await session.getCurrentURL()
        let retrySiteName = site == .joefortune ? "joe" : "ignition"
        let retrySettlementContext = VisionContext(site: retrySiteName, phase: .settlement, currentURL: retryURL, attemptNumber: 1)
        let retrySettlement = await aiSettlement.waitForSettlement(
            captureScreenshot: { await session.captureScreenshot() },
            context: retrySettlementContext,
            maxTimeoutMs: 15000,
            sessionId: label
        )
        log("V5.2 [\(label)] Retry AI settlement: \(retrySettlement.reasoning) (\(retrySettlement.durationMs)ms)")
