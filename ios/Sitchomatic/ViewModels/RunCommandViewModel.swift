...
class RunCommandViewModel {
    ...
    private var lastCompletedSnapshot: Int = 0
    ...
    var siteMode: String {
        switch activeMode {
        case .login:
            if loginVM.batchSiteLabel.lowercased().contains("ignition") { return "ignition" }
            if loginVM.batchSiteLabel.lowercased().contains("dual") || loginVM.batchSiteLabel.lowercased().contains("unified") { return "double" }
            return "joe"
    ...
    var batchStartTime: Date? {
        switch activeMode {
        case .login: loginVM.batchStartTime
        case .ppsr: nil
        case .none: nil
        }
    }
    ...
    var speedPerMinute: Double {
        ...
        return Double(completedCount) / (elapsed / 60.0)
    }

    var speedPerMinuteFormatted: String {
        let speed = speedPerMinute
    ...
    var activeWorkerCount: Int {
        switch activeMode {
        case .login: loginVM.attempts.filter { !$0.status.isTerminal }.count
        case .ppsr: ppsrVM.checks.filter { !$0.status.isTerminal }.count
        case .none: 0
        }
    }

    var currentCredentialLabel: String {
        switch activeMode {
        case .login:
            if let current = loginVM.attempts.first(where: { !$0.status.isTerminal }) {
                return current.credential.username
            }
            return ""
        case .ppsr:
            if let current = ppsrVM.checks.first(where: { !$0.status.isTerminal }) {
                return "•••\(current.card.number.suffix(4))"
            }
            return ""
        case .none: return ""
        }
    ...
    var recentResults: [RecentResultItem] {
        switch activeMode {
        case .login:
            return loginVM.attempts
                .filter { $0.status.isTerminal }
                ...
                .map { attempt in
                    RecentResultItem(
                        ...
                        isSuccess: attempt.credential.status == .working
                    )
                }
        case .ppsr:
            return ppsrVM.checks
                ...
                .map { check in
                    RecentResultItem(
                        id: check.id.uuidString,
                        label: "\(check.card.brand.rawValue) •••\(check.card.number.suffix(4))",
                        reason: check.errorMessage ?? (check.status == .completed ? "Alive" : "Dead"),
                        resultStatus: check.status == .completed ? "Working" : "Dead",
                        isSuccess: check.status == .completed
                    )
                }
        case .none:
            return []
...
struct RecentResultItem: Identifiable {
    let id: String
    let label: String
    let reason: String
    let resultStatus: String
