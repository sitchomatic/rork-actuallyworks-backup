...
class LoginCredential: Identifiable {
    ...
    func recordResult(success: Bool, duration: TimeInterval, error: String? = nil, detail: String? = nil) {
        ...
        if success {
            ...
            if detailLower.contains("has been disabled") || detailLower.contains("permdisabled") {
                status = .permDisabled
            } else if detailLower.contains("temporarily disabled") || detailLower.contains("tempdisabled") {
                status = .tempDisabled
            } else {
                status = .noAcc
            }
